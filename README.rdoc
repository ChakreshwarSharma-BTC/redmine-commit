...
3)In application.rb, config the secret_token field , for eg: config.secret_token: <%= ENV['GITHUB_REPO_TOKEN'] %>, Use the same token while creating webhook on github
